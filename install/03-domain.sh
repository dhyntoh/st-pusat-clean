...
setup_domain() {
    ...
    if [[ "$host_choice" == "1" ]]; then
        ...
        echo "IP=$DOMAIN" >> /var/lib/kyt/ipvps.conf 2>/dev/null
    else
        info "Using IP-based setup. Get a domain later via menu."
        DOMAIN=$(get_ip)
        echo "$DOMAIN" > /etc/xray/domain
    fi
...
setup_ssl() {
    ...
    ~/.acme.sh/acme.sh --issue -d "$domain" --standalone -k ec-256 --server letsencrypt 2>&1

    if [[ $? -eq 0 ]]; then
        ~/.acme.sh/acme.sh --installcert -d "$domain" --fullchainpath /etc/xray/xray.crt \
        ...
        warn "SSL failed, using self-signed fallback"
        openssl req -x509 -nodes -days 365 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 \
            -keyout /etc/xray/xray.key -out /etc/xray/xray.crt \
            -subj "/CN=$domain" 2>/dev/null
    fi
